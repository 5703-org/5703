...
import re
import numbers
from .runtime import query_execution_config
...
class E5Embedding:
    def __init__(
        ...
        if device is not None and device not in ("cpu", "cuda", "cuda:0", "mps"):
            raise ValueError("Embedding device must explicitly select cpu, cuda, cuda:0 or mps")
        options = {}
...
def make_embedding(config, *, runtime_device=None):
    config = query_execution_config(config, runtime_device)
