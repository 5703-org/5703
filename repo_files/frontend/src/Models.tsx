...
export function ModelsPage({ onActivated }: { onActivated: () => void }) {
  ...
  const tested = (value: ModelTest) => {
    setState(current => current && { ...current, items: current.items.map(item => item.id === value.configuration_id ? { ...item, latest_test: value, ...(value.role === 'checker' ? { latest_checker_test: value } : { latest_answer_test: value }) } : item) });
    setNotice('');
  };
  ...
  return <div className="content-page settings-page">
    ...
    <p className="page-intro">Choose an answering service, save its settings, test basic generation, structured output and the project contract, then enable the tested answer and checker versions.</p>
...
function ModelEditor({ initial, presets, state, onSaved, onTested, onActivated }: { initial: ModelConfiguration | null; presets: ModelPreset[]; state: ModelState; onSaved: (value: ModelConfiguration) => void; onTested: (value: ModelTest) => void; onActivated: (value: ModelState) => void }) {
  ...
  const [testRole, setTestRole] = useState<'answer' | 'checker'>('answer');
  const [testTier, setTestTier] = useState<'basic' | 'structured' | 'project' | 'all'>('all');
  const [checkerId, setCheckerId] = useState(initial?.id || '');
  const [history, setHistory] = useState<ModelTest[] | null>(null);
  const [networkLabel, setNetworkLabel] = useState('local_unverified');
  const pendingTestKey = useRef<string | null>(null);
  const answerTest = initial?.latest_answer_test;
  const checker = state.items.find(item => item.id === checkerId) || initial;
  const checkerTest = checker?.latest_checker_test;
  const passedProject = (value: ModelTest | null | undefined, id: string | undefined) => value?.test_version === 'provider_probe_v2' && value.status === 'passed' && value.project_passed === true && value.configuration_id === id;
  const loadProfile = async () => {
    try {
      const result = await post<{ capability_version: 'provider_capabilities_v1'; capabilities: NonNullable<ModelValues['capabilities']>; suggested_options: Partial<ModelValues> }>(`${root}/capability-profile`, config);
      setConfig(previous => ({ ...previous, ...result.suggested_options, capability_version: result.capability_version, capabilities: result.capabilities })); setError(null);
    } catch (caught) { setError(caught); }
  };
  const changeCapability = <K extends keyof NonNullable<ModelValues['capabilities']>>(field: K, value: NonNullable<ModelValues['capabilities']>[K]) => setConfig(previous => previous.capabilities ? { ...previous, capabilities: { ...previous.capabilities, [field]: value, declaration: 'operator_declared' } } : previous);
  const changedDestination = initial?.has_api_key && endpointIdentity(initial.config) !== endpointIdentity(config);
  const canActivate = !!initial && !dirty && !error && (!initial.active || state.active_checker_configuration_id !== checker?.id) && passedProject(answerTest, initial.id) && passedProject(checkerTest, checker?.id);
  ...
  const action = async (kind: 'save' | 'test' | 'activate') => {
    ...
    try {
      if (kind === 'save') onSaved(await post<ModelConfiguration>(initial ? `${root}/${initial.id}/versions` : root, { name: name.trim(), preset, config: { ...config, model: config.model.trim(), base_url: config.base_url?.trim() || null, api_version: config.api_version?.trim() || null }, ...(key ? { api_key: key } : {}), ...(clearKey ? { clear_api_key: true } : {}) }));
        ...
        pendingTestKey.current ||= crypto.randomUUID();
        const value = await post<ModelTest>(`${root}/${initial!.id}/test`, { role: testRole, tier: testTier, network_label: networkLabel }, pendingTestKey.current);
        onTested(value); pendingTestKey.current = null; setHistory(null);
      }
      else onActivated(await post<ModelState>(`${root}/${initial!.id}/activate`, { test_id: answerTest!.id, checker_configuration_id: checker!.id, checker_test_id: checkerTest!.id, expected_active_version: state.active_version }));
    } catch (caught) { setError(caught); }
  ...
  return <section className="model-editor"><h2>{initial ? `${initial.name} · version ${initial.revision}` : 'New configuration'}</h2>
    <form className="stack" onSubmit={submit}><fieldset disabled={!!busy} className="stack">
      ...
      <details className="model-advanced"><summary>Advanced settings</summary><div className="stack">
        ...
        <label>Reasoning effort<input maxLength={30} value={config.reasoning_effort || ''} onChange={event => change('reasoning_effort', event.target.value || null)} placeholder="Provider default" /><small>Set only a value supported by this model and protocol.</small></label>
        <Button onClick={loadProfile}>Load protocol profile</Button><p className="secondary">Loads explicit suggested capabilities for this provider and model. Review them, save a new version, then test. A profile is a declaration, not a successful connection.</p>
        {config.capabilities && <><label>API style<select value={config.capabilities.api_style} onChange={event => { const style = event.target.value as NonNullable<ModelValues['capabilities']>['api_style']; setConfig(previous => ({ ...previous, capabilities: { ...previous.capabilities!, api_style: style, output_token_parameter: style === 'responses' ? 'max_output_tokens' : style === 'gemini_generate' ? 'maxOutputTokens' : style === 'anthropic_messages' ? 'max_tokens' : previous.token_limit_parameter || 'max_tokens', declaration: 'operator_declared' } })); }}><option value="chat_completions">Chat Completions</option><option value="responses">Responses API</option><option value="anthropic_messages">Anthropic Messages</option><option value="gemini_generate">Gemini Generate Content</option></select></label>
        <div className="form-grid"><label>Probe output reservation<input type="number" min={128} max={8192} value={config.capabilities.probe_output_tokens ?? 512} onChange={event => changeCapability('probe_output_tokens', Number(event.target.value))} /></label><label>Declared output ceiling<input type="number" min={1024} max={200000} value={config.capabilities.output_token_ceiling ?? 32768} onChange={event => changeCapability('output_token_ceiling', Number(event.target.value))} /></label></div>
        <div className="form-grid">{(['temperature', 'seed', 'reasoning_effort', 'thinking_toggle'] as const).map(field => <label className="inline-check" key={field}><input type="checkbox" checked={!!config.capabilities?.[field]} onChange={event => changeCapability(field, event.target.checked)} />Protocol supports {titleCase(field)}</label>)}</div>
        <DataDetails title="Frozen capability declaration" data={config.capabilities} /></>}
      </div></details>
    </fieldset>
    ...
    {initial && <div className="stack"><h3>Compatibility tests</h3><div className="form-grid"><label>Test role<select value={testRole} disabled={!!busy || !!pendingTestKey.current} onChange={event => setTestRole(event.target.value as 'answer' | 'checker')}><option value="answer">Answer generation</option><option value="checker">Semantic checker</option></select></label><label>Test stage<select value={testTier} disabled={!!busy || !!pendingTestKey.current} onChange={event => setTestTier(event.target.value as typeof testTier)}><option value="all">All three stages</option><option value="basic">Basic generation</option><option value="structured">Structured output</option><option value="project">Project contract</option></select></label><label>Network label<input maxLength={80} value={networkLabel} disabled={!!busy || !!pendingTestKey.current} onChange={event => setNetworkLabel(event.target.value)} /><small>Operator-provided context; the application cannot verify your physical network.</small></label></div>
    <label>Checker configuration<select value={checkerId || initial.id} disabled={!!busy} onChange={event => setCheckerId(event.target.value)}>{state.items.map(item => <option key={item.id} value={item.id}>{item.name} · version {item.revision}{item.id === initial.id ? ' (same saved configuration)' : ''}</option>)}</select></label><p className="secondary">Answer project test: {passedProject(answerTest, initial.id) ? 'passed' : 'required'} · Checker project test: {passedProject(checkerTest, checker?.id) ? 'passed' : 'required'}. Test a different checker by selecting its saved configuration above and choosing the checker role.</p></div>}
    ...
    <p className="secondary">{config.provider === 'mock' ? 'Mock checks run locally. They do not test a live provider or establish answer quality.' : 'Tests use authored public fixtures and may consume provider quota. All stages reserve at most three calls and 180 seconds per selected role, without automatic retries. Checker project output reserves at least 4,096 tokens. A pass proves that tested contract, not answer quality.'}</p>
    {currentTest && <section className={`model-test-result ${currentTest.status}`} aria-label="Latest connection test" role="status"><h3>{currentTest.test_version !== 'provider_probe_v2' ? 'Legacy connection test' : currentTest.model_mode === 'mock' ? 'Mock check' : 'Connection test'} {currentTest.status}</h3><p>{currentTest.message}</p><p className="secondary">{currentTest.diagnostic_code} · {currentTest.latency_ms} ms · {currentTest.created_at}</p>{currentTest.status === 'failed' && <p>{modelDiagnostic(currentTest.diagnostic_code)}</p>}{dirty && <p>This result belongs to the saved version; your unsaved edits have not been tested.</p>}<p>{currentTest.role || 'answer'} · {currentTest.tier || 'legacy'} · {currentTest.network_label || 'Network not recorded'}</p>{currentTest.test_version !== 'provider_probe_v2' && <p>This historical test does not establish the three-stage project or checker contract.</p>}{currentTest.stages?.map((stage, index) => <DataDetails key={index} title={`Stage ${index + 1}: ${String(stage.tier)} · ${String(stage.status)}`} data={stage} />)}<DataDetails title="Test usage and budget" data={{ usage: currentTest.usage, budget: currentTest.budget }} /></section>}
    {initial && <><Button onClick={async () => { try { setHistory(await request<ModelTest[]>(`${root}/${initial.id}/tests`)); } catch (caught) { setError(caught); } }}>Load test history</Button>{history && <DataDetails title="Compatibility history (latest 100 per versioned/legacy type)" data={history} />}</>}
