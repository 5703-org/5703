...
function AnswerView({ answer, working, onEvidence, onFeedback, onRegenerate, onSuggestion }: { answer: Answer; working: boolean; onEvidence: (id?: string) => void; onFeedback: () => void; onRegenerate: () => void; onSuggestion: (text: string) => void }) {
  ...
  const sourceIds = [...new Set([...answer.response.citations, ...answer.evidence.map(item => item.evidence_id)])];
...
export function EvidenceViewer({ answer, initialId, onClose }: { answer: Answer; initialId?: string; onClose: () => void }) {
  const sourceIds = [...new Set([...answer.response.citations, ...answer.evidence.map(item => item.evidence_id)])];
  const [selected, setSelected] = useState(initialId || sourceIds[0]);
  ...
  return <Modal title="Sources" kind="evidence-modal" onClose={onClose}>{sourceIds.length > 1 && <nav className="source-tabs" aria-label="Select source">{sourceIds.map((sourceId, i) => <Button className={selected === sourceId ? 'active' : ''} key={sourceId} onClick={() => setSelected(sourceId)}>Source {i + 1}</Button>)}</nav>}{loading ? <Loading>Loading the original passage…</Loading> : error ? <><h3>Source unavailable</h3><ErrorNotice error={error} /><p className="secondary">This response keeps its original citation. A different passage has not been substituted.</p></> : evidence ? <><span className="eyebrow">Original source passage</span><h3 className="source-title">{evidence.source_title}</h3><p className="source-locator">{evidence.section}{evidence.section ? ' · ' : ''}{evidence.locator}{evidence.pages.length > 0 ? ` · Page${evidence.pages.length > 1 ? 's' : ''} ${evidence.pages.join(', ')}` : ''}</p>{originalSourceUrl && <div className="source-attribution"><p>OpenStax / Rice University · Access for free at openstax.org</p><a className="source-original-link" href={originalSourceUrl} target="_blank" rel="noopener noreferrer">View original source<ExternalLink size={15} aria-hidden="true" /></a></div>}<div className="source-passage">{evidence.text}</div><DataDetails data={{ request_id: answer.request_id, evidence_id: evidence.evidence_id, chunk_id: evidence.chunk_id, asset_id: evidence.asset_id, processing_id: evidence.processing_id, text_hash: evidence.text_hash, source_url: evidence.source_url, license: evidence.license, context_order: evidence.context_order, inherited_from: evidence.inherited_from }} /></> : <p>No sources were supplied for this response.</p>}</Modal>;
}
