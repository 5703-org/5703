...
import { ControlledEvidenceViewer } from './ControlledEvidence';
import { ObservedPresentation } from './Exposure';
import { MemorySavedNotice, SessionMemoryNotices } from './Memory';
import type { TeachingMode, TeachingRequest } from './learning-types';
import { responseSources, sourceLocator } from './sources';
import type { Answer, AnswerMode, Capabilities, ChatResponse, Evidence, Feedback, Job, JobReceipt, MCQResponse, Message, Profile, Session } from './types';
...
const answerModeKey = (userId: string, sessionId?: string) => `${draftKey(userId, sessionId)}.answer-mode`;
const readAnswerMode = (value: string | null): AnswerMode => value === 'general_knowledge' ? 'general_knowledge' : 'textbook';
const teachingKey = (userId: string, sessionId?: string) => `${draftKey(userId, sessionId)}.teaching`;
function readTeaching(value: string | null): TeachingRequest {
  try {
    const parsed = JSON.parse(value || '{}') as TeachingRequest;
    return {
      ...(parsed.teaching_mode === 'direct' || parsed.teaching_mode === 'hint' ? { teaching_mode: parsed.teaching_mode } : {}),
      ...(['new', 'more_hint', 'full_explanation', 'continue'].includes(parsed.task_action || '') ? { task_action: parsed.task_action } : {}),
...
export function Chat({ sessionId, navigating = false, userId, profile, capabilities, onCreated, refreshSessions, onOverlay }: { sessionId?: string; navigating?: boolean; userId: string; profile: Profile | null; capabilities: Capabilities | null; onCreated: (id: string) => void; refreshSessions: () => Promise<void>; onOverlay: (open: boolean) => void }) {
  ...
  const [answerMode, setAnswerMode] = useState<AnswerMode>(() => readAnswerMode(sessionStorage.getItem(answerModeKey(userId, sessionId))));
  const [teaching, setTeaching] = useState<TeachingRequest>(() => readTeaching(sessionStorage.getItem(teachingKey(userId, sessionId))));
  ...
  const [evidenceTarget, setEvidenceTarget] = useState<{ answer: Answer; evidenceId?: string; claimId?: string } | null>(null);
  ...
  useEffect(() => { sessionStorage.setItem(answerModeKey(userId, sessionId), answerMode); }, [answerMode, userId, sessionId]);
  useEffect(() => { sessionStorage.setItem(teachingKey(userId, sessionId), JSON.stringify(teaching)); }, [teaching, userId, sessionId]);
      ...
      try {
        ...
        if (!activeStates.has(current.state)) {
          if (current.state === 'failed' || current.state === 'cancelled') {
            ...
            if (previous && !draftRef.current) { setDraft(previous); setAnswerMode(readAnswerMode(sessionStorage.getItem(`${pendingKey(userId, sessionId)}.answer-mode`))); setTeaching(readTeaching(sessionStorage.getItem(`${pendingKey(userId, sessionId)}.teaching`))); }
          }
          sessionStorage.removeItem(pendingKey(userId, sessionId));
          sessionStorage.removeItem(`${pendingKey(userId, sessionId)}.answer-mode`);
          sessionStorage.removeItem(`${pendingKey(userId, sessionId)}.teaching`);
  ...
  const send = async (content = draft, instruction: TeachingRequest = teaching) => {
    ...
    try {
      ...
      const signature = JSON.stringify([targetSession, content.trim(), useProfile, answerMode, instruction]);
      if (pendingSubmission.current?.signature !== signature) pendingSubmission.current = { signature, key: crypto.randomUUID() };
      const receipt = Object.keys(instruction).length
        ? await api.send(targetSession, content.trim(), useProfile, pendingSubmission.current!.key, answerMode, instruction)
        : await api.send(targetSession, content.trim(), useProfile, pendingSubmission.current!.key, answerMode);
      sessionStorage.setItem(pendingKey(userId, targetSession), content);
      sessionStorage.setItem(`${pendingKey(userId, targetSession)}.answer-mode`, answerMode);
      sessionStorage.setItem(`${pendingKey(userId, targetSession)}.teaching`, JSON.stringify(instruction));
      sessionStorage.setItem(answerModeKey(userId, targetSession), answerMode);
      sessionStorage.setItem(teachingKey(userId, targetSession), '{}');
      sessionStorage.setItem(draftKey(userId, targetSession), '');
      setDraft(''); setTeaching({}); setJob(null); setJobId(receipt.job_id); pendingSubmission.current = null;
      if (!sessionId) { sessionStorage.removeItem(answerModeKey(userId)); sessionStorage.removeItem(teachingKey(userId)); onCreated(targetSession); } else { await loadTranscript(); await refreshSessions(); }
    } catch (caught) { if (mounted.current) { setError(caught); if (!draftRef.current) { setDraft(content); setTeaching(instruction); } } }
  ...
  const latestAnswer = messages.filter(item => item.answer).at(-1)?.answer;
  const displayedTeachingMode = teaching.teaching_mode || latestAnswer?.teaching_mode || 'direct';
  return <div className="chat-workspace">
    ...
    <div className="composer-region"><div className="composer-column">{readonly ? <div className="archived-notice"><Archive size={18} /><p>This conversation is archived.</p><Button onClick={archive}>Restore to continue</Button></div> : <form className="composer" onSubmit={event => { event.preventDefault(); void send(); }}>
      <label className="sr-only" htmlFor="chat-composer">Message Learning Assistant</label>
      <textarea ref={textarea} disabled={navigating} id="chat-composer" rows={1} placeholder="Ask a question, or explore an idea…" value={draft} onChange={event => { setDraft(event.target.value); setError(null); }} onKeyDown={keyDown} aria-describedby="composer-help" />
      <div className="composer-toolbar"><div className="composer-options">
        <label className="answer-source-control"><span className="sr-only">Answer source</span><select value={answerMode} disabled={working} onChange={event => setAnswerMode(readAnswerMode(event.target.value))}><option value="textbook">Textbook sources</option><option value="general_knowledge">General knowledge</option></select></label>
        <label className="answer-source-control"><span className="sr-only">Teaching style</span><select value={displayedTeachingMode} disabled={working} onChange={event => setTeaching({ ...teaching, teaching_mode: event.target.value as TeachingMode })}><option value="direct">Direct explanation</option><option value="hint">Get a hint</option></select></label><label className="profile-toggle"><input type="checkbox" checked={useProfile} onChange={e => setUseProfile(e.target.checked)} /><span>{useProfile ? `${titleCase(profile?.level || 'intermediate')} explanations` : 'Profile off'}</span></label>
      </div><div className="send-area">{draft.length > 3600 && <span className={draft.length > 4000 ? 'error-text' : 'secondary'}>{draft.length}/4,000</span>}{jobId ? <IconButton label="Stop generation" className="primary send-button" onClick={stop}><Square size={17} fill="currentColor" /></IconButton> : <IconButton type="submit" label="Send message" className="primary send-button" disabled={!draft.trim() || submitting || navigating || draft.trim().length > 4000}><ArrowUp size={20} /></IconButton>}</div></div>
    </form>}{!readonly && <div className="task-context">{teaching.task_action === 'new' ? <><span>Next message starts a new problem.</span> <Button disabled={working} onClick={() => setTeaching({})}>Continue current problem</Button></> : <>{displayedTeachingMode === 'hint' && <span>Hints continue within this problem. </span>}{latestAnswer?.task_id && <Button disabled={working} onClick={() => { setTeaching({ teaching_mode: 'direct', task_action: 'new' }); textarea.current?.focus(); }}>Start a new problem</Button>}</>}{!useProfile && <span> Profile and memory are off for this turn.</span>}</div>}<p id="composer-help" className="composer-note">{answerMode === 'general_knowledge' ? 'General knowledge uses the model’s knowledge without textbook sources. Verify important details.' : capabilities?.model_mode === 'mock' ? 'Demo responses use a mock model. Check the sources and verify important details.' : 'Answers can be imperfect. Use the sources to check your understanding.'}</p></div></div>
    {evidenceTarget && <EvidenceViewer answer={evidenceTarget.answer} initialId={evidenceTarget.evidenceId} claimId={evidenceTarget.claimId} onClose={() => setEvidenceTarget(null)} />}
...
function AnswerView({ answer, working, onEvidence, onFeedback, onRegenerate, onSuggestion, onTeaching }: { answer: Answer; working: boolean; onEvidence: (id?: string, claimId?: string) => void; onFeedback: () => void; onRegenerate: () => void; onSuggestion: (text: string) => void; onTeaching?: (action: 'more_hint' | 'full_explanation') => void }) {
  ...
  const generalKnowledge = answer.answer_mode === 'general_knowledge';
  ...
  return <><div className="answer-metadata"><span>{answer.model_mode === 'mock' ? 'Demo (mock model)' : 'Live model'}</span><span>·</span><span>{level ? `${level} profile applied` : 'Profile not applied'}</span><span className="status-tag">{generalKnowledge ? 'General knowledge' : 'Textbook sources'}</span>{isChat && response.response_type === 'refusal' && <span className="status-tag">{generalKnowledge ? 'Unable to answer' : 'Evidence limitation'}</span>}{isChat && response.response_type === 'clarification' && <span className="status-tag">A quick clarification</span>}{!isChat && <span className="status-tag">Historical MCQ result</span>}</div>
    {generalKnowledge && <p className="source-basis">Model knowledge · verify independently.</p>}
    {answer.teaching_mode === 'hint' && <p className="source-basis">Hint · help level {answer.help_level || 0}</p>}
    <ObservedPresentation answerId={answer.id} exposure={answer.presentation?.id ? { presentation_id: answer.presentation.id, surface: 'answer' } : null}><SafeMarkdown text={text} evidence={generalKnowledge ? [] : answer.evidence} citationIds={sourceIds} claims={answer.attribution?.claims || []} onEvidence={onEvidence} /></ObservedPresentation>
    {answer.teaching_mode === 'hint' && onTeaching && <div className="task-controls" aria-label="Help for this problem"><Button disabled={working} onClick={() => onTeaching('more_hint')}>Another hint</Button><Button disabled={working} onClick={() => onTeaching('full_explanation')}>Show full explanation</Button></div>}
    {answer.memory_notices?.filter(notice => notice.action === 'undo_save').map(notice => <MemorySavedNotice key={`${notice.memory_id}:${notice.version}`} memoryId={notice.memory_id} version={notice.version} />)}
    <div className="answer-actions">{sourceIds.length > 0 && <Button onClick={() => onEvidence()}><BookOpen size={16} />{citationsRecorded ? 'Sources' : 'Stored sources'} <span className="source-count">{sourceIds.length}</span></Button>}<Button onClick={copy}>{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? 'Copied' : 'Copy'}</Button><Button onClick={onFeedback}><ThumbsUp size={16} />Feedback</Button>{answer.can_regenerate && <Button onClick={onRegenerate} disabled={working}><RotateCcw size={16} />Regenerate</Button>}</div>
    <ErrorNotice error={copyError} />{isChat && response.follow_up_questions.length > 0 && <div className="followups" aria-label="Follow-up suggestions">{response.follow_up_questions.map(question => <Button disabled={working} key={question} onClick={() => onSuggestion(question)}>{question}<ArrowUp size={15} /></Button>)}</div>}
    <details className="applied-details"><summary>Applied preferences and response details</summary><DataDetails title="Profile snapshot" data={nestedProfile ? { level: nestedProfile.level, style: nestedProfile.style, language: nestedProfile.language, topics: nestedProfile.topics } : { applied: false }} /><DataDetails title="Response record" data={{ answer_id: answer.id, request_id: answer.request_id, mode: answer.mode, answer_mode: answer.answer_mode || 'textbook', source_provenance: answer.source_provenance || 'textbook_evidence', response_schema: answer.response_schema, teaching_mode: answer.teaching_mode || 'direct', task_id: answer.task_id || null, help_level: answer.help_level || 0, presentation_id: answer.presentation?.id || null, timing: answer.timing }} /></details></>;
}
...
export function EvidenceViewer({ answer, initialId, claimId, onClose }: { answer: Answer; initialId?: string; claimId?: string; onClose: () => void }) {
  if (answer.presentation?.id) return <ControlledEvidenceViewer answer={answer} initialId={initialId} claimId={claimId} onClose={onClose} />;
  if (answer.teaching_mode === 'hint') return <Modal title="Sources" kind="evidence-modal" onClose={onClose}><p>The approved preview for this hint is unavailable. Reload the conversation before opening its source.</p></Modal>;
  return <LegacyEvidenceViewer answer={answer} initialId={initialId} onClose={onClose} />;
}

function LegacyEvidenceViewer({ answer, initialId, onClose }: { answer: Answer; initialId?: string; onClose: () => void }) {
  const { ids: sourceIds, recorded: citationsRecorded } = responseSources(answer);
