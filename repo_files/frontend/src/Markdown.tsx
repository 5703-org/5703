...
export function SafeMarkdown({ text, evidence = [], citationIds, onEvidence }: { text: string; evidence?: Evidence[]; citationIds?: string[]; onEvidence?: (id: string) => void }) {
  ...
  const components = useMemo<Components>(() => ({
    a: ({ href, children }) => href?.startsWith('#evidence-') && openEvidence.current ? <button className="citation" aria-label={`Open source ${String(children).replace(/[\[\]]/g, '')}`} onClick={() => openEvidence.current?.(href.slice(10))}>{children}</button> : <a href={href} target="_blank" rel="noopener noreferrer">{children}</a>,
