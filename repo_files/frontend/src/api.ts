import type { Answer, Capabilities, Evidence, Feedback, Job, JobReceipt, MessagePage, Profile, Session, User } from './types';
...
export async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  catch (error) { if (error instanceof DOMException && error.name === 'AbortError') throw error; throw new ApiError('CONNECTION_ERROR', 'We could not connect to the server. Your draft is still here.'); }
...
export const api = {
  ...
  send: (id: string, content: string, useProfile: boolean, key: string) => post<JobReceipt>(`/sessions/${id}/messages`, { content, use_profile: useProfile }, key),
