...
## Enhancement-specific checks — 20 September

The independent source and teaching selects wrap within the existing composer toolbar. Task controls and memory notices wrap as ordinary text/actions. They retain 44px control targets and readable labels at narrow widths. Source highlights inherit the existing text size, use a visible background and underline, preserve exact Unicode text, and keep wide source blocks within the drawer's scroll region. The memory page uses the existing responsive forms and native dialog focus/close behavior.

Extend UI-04/06/09/10/11/12 evidence with claim-specific multi-span text, escaped source markup, hint preview and explicit complete-source disclosure, source permission failures, memory opt-in/edit/source/delete/undo, direct/hint/new-task controls and draft recovery. Verify that unexpanded hint source URLs and complete passages are absent from the DOM and response projection rather than merely clipped. Record actual desktop and 390px controls, keyboard open/Tab/Escape/focus return, source scroll reachability and persisted task/memory state when the integrated runtime is available. Keep physical-device/IME/assistive-technology and independent learning measurements separately scoped.
