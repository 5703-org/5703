...
def test_postgres_teaching_resolves_frozen_managed_key_after_active_rotation(
    runtime, evaluation_backend, tmp_path, monkeypatch
):
    """Actual teaching jobs plus an observed adapter seam; no external provider."""
    from cryptography.fernet import Fernet
    from app.modules.experiment import bridge
    from app.modules.model_settings import service as model_settings
    ...
    with runtime.db() as db:
        ...
        actor_id = actor.id
        payload = {
            "name": "Frozen teaching credential fixture",
            "config": {"provider": "mock", "model": "authored-extractive-v1"},
            "api_key": "teaching-frozen-fixture-key",
        }
        original = model_settings.save(
            db, runtime.settings, actor, ConfigurationSave.model_validate(payload)
        )
        config_row = knowledge.config_create(
            db,
            "evaluation",
            "Explicit managed teaching fixture",
            {"model_configuration_id": model_settings.PREFIX + original["id"]},
        )
        run = bridge.create_teaching_run(
            db, runtime.settings, actor, [base_id], configuration_id=config_row.id
        )
        run_id = run.id
        db.commit()
        rotated = model_settings.save(
            db,
            runtime.settings,
            actor,
            ConfigurationSave.model_validate(
                {**payload, "api_key": "teaching-replacement-fixture-key"}
            ),
            original["id"],
        )
        check = model_settings.run_test(db, runtime.settings, actor, rotated["id"])
        checker = model_settings.run_test(
            db, runtime.settings, actor, rotated["id"], CompatibilityInput(role="checker")
        )
        for receipt in (check, checker):
            assert receipt["project_passed"] and receipt["model_mode"] == "mock"
            assert [stage["tier"] for stage in receipt["stages"]] == [
                "basic",
                "structured",
                "project",
            ]
        current = model_settings.state(db, runtime.settings, actor)["active_version"]
        model_settings.activate(
            db,
            runtime.settings,
            actor,
            rotated["id"],
    ...
    def observed_adapter(config, *, api_key):
        observed.append((config.configuration_id, api_key))
        return LLMAdapter(config, api_key=api_key)

    monkeypatch.setattr(bridge, "LLMAdapter", observed_adapter)
    response = runtime.client.post(
        f"/api/v1/experiments/{run_id}/start", headers=runtime.headers("admin@example.com")
    )
    assert response.status_code == 202
    for _ in range(9):
        assert runtime.work()
    assert observed == [(model_settings.PREFIX + original["id"], "teaching-frozen-fixture-key")] * 9
    with runtime.db() as db:
        results = bridge.run_results(db, db.get(ExperimentRun, run_id))
        assert results["outcome_counts"] == {"completed": 9}
        assert "teaching-frozen-fixture-key" not in str(results)
        actor = db.get(User, actor_id)
        current = model_settings.state(db, runtime.settings, actor)["active_version"]
        model_settings.activate(db, runtime.settings, actor, None, None, current)
