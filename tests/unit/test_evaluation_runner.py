...
def test_duplicate_candidates_and_split_counts(tmp_path):
    ...
    path = write_sciq_fixture(tmp_path / "authored-sciq.json")
...
def config(tmp_path, **changes):
    return {
        ...
        "condition": "E1",
        "dataset_path": changes["dataset_path"]
        if "dataset_path" in changes
        else str(write_sciq_fixture(tmp_path.parent / (tmp_path.name + "-authored-sciq.json"))),
...
def test_submission_attempt_limit_preserves_next63_items_after_uncertain_receipt(tmp_path):
    source = tmp_path / "authored64.json"
    source.write_text(
        json.dumps(
            [
                {
                    "question_id": f"authored-{index}",
                    "question": "Where is oxygen produced?",
                    "correct_answer": "chloroplast",
                    "distractor1": "nucleus",
                    "distractor2": "ribosome",
                    "distractor3": "mitochondrion",
                    "support": "PRIVATE_AUTHORED_SUPPORT",
                }
    ...
    class FailedSubmissionBackend(DurableBackend):
        def __init__(self):
            super().__init__()
            self.attempted = []

        def submit(self, command, **kwargs):
            self.attempted.append(command["item_id"])
            raise ConnectionError("Authored missing receipt")

    backend = FailedSubmissionBackend()
    run = EvaluationRun.freeze(
        config(tmp_path / "runs", dataset_path=str(source), limit=64), backend
    )
    state = run.advance(backend, max_new_submissions=1)
    assert backend.attempted == ["item_000000"]
    assert state["status"] == "needs_reconciliation"
    assert [row["status"] for row in state["items"]] == ["uncertain"] + ["pending"] * 63
    resumed = EvaluationRun(run.directory)
    resumed.advance(backend, max_new_submissions=0)
    assert backend.attempted == ["item_000000"]
