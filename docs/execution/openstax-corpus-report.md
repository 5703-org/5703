...
The per-book acquisition/processing details below retain the 8 September observation and original source evidence. On 13 September, the same active release was rechecked during additive migration and isolated portable import; real DeepSeek answering is now enabled in the existing installation. No replacement downloads or synthetic vectors were introduced. Counts are units unless explicitly marked pages or chunks; unresolved blocking units and retained historical issue codes are different quantities.

Current verification: [preserved source/corpus/history fingerprints](../../evidence/answering-upgrade/20260913/migration-preservation.json), [isolated corpus import](../../evidence/answering-upgrade/20260913/portable-corpus-checked-import.json), [current per-book database counts](../../evidence/answering-upgrade/20260913/current-per-book-counts.json), [actual model questions and original cited passages](../../evidence/answering-upgrade/20260913/live-suite-attempt3.json). Existing database: `127.0.0.1:15432/learning`; the 55432 endpoint below is its recorded earlier environment. The complete portable package stores originals/recovery files and `corpus.jsonl.gz` under `resources/official-corpus`, with a file-hash manifest. Installation copies source files to its own `artifacts/storage` and imports into its separate PostgreSQL database. Every book retains the same edition, license, chunk count, vector dimension and immutable source hash reported below. Full-book diagram/formula semantics and independent human source review remain unverified.
