...
function Workspace({ user, setUser, logout, path }: { user: User; setUser: (user: User) => void; logout: () => void; path: string }) {
  ...
  useEffect(() => { Promise.all([api.profile(), api.capabilities()]).then(([p, c]) => { setProfile(p); setCapabilities(c); }).catch(setConfigError); }, []);
